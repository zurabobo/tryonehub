...
export default function Home() {
  ...
  const filtered = [...sitesData.categories]
    .filter((cat) => {
      const q = search.toLowerCase();
  ...
  return (
    <div className="grid grid-rows-[auto_auto_1fr] p-4">
      <h1
        className={`
          text-center text-3xl font-medium mt-[10px] mb-4 font-[mrgvlovani]
          ${theme === "dark" ? "text-[#E5E7EB]" : "text-[#0F172A]"}
        `}
      >
        {t("home_title1")}
      </h1>
      ...
      <input
        ...
        onChange={(e) => setSearch(e.target.value)}
        className={`
          w-full max-w-[448px] mx-auto mb-4 p-2 text-[13px] rounded-lg outline-none border transition-colors duration-200
          ${
            theme === "dark"
              ? "bg-[#E5E7EB] text-[#0F172A] border-transparent"
